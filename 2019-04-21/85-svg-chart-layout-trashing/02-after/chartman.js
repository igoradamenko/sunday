;(function() {
  ...
  Chartman.prototype.fallbackRescaleChart = function(xScale, yScale) {
    ...
    for (var key in self.lines) {
      DOM.write(function(key, d) {
        self.paths.chart[key].setAttribute('d', d);
      }, key, Architect.getD(self.lines[key], widthCost, heightCost));
    }
  };
  ...
  Chartman.prototype.fallbackRescaleRange = function(yScale) {
    ...
    for (var key in self.lines) {
      DOM.write(function(key, d) {
        self.paths.range[key].setAttribute('d', d);
      }, key, Architect.getD(self.lines[key], self.rangeCellWidth, heightCost));
    }
  };
  ...
  Chartman.prototype.onRangeWindowResize = function(e, direction) {
    ...
    function onMouseLeftMove(e) {
      ...
      recalcChartScale(newWidth, newLeft, function() {
        DOM.write(function() {
    ...
    function onMouseRightMove(e) {
      ...
      recalcChartScale(newWidth, currentLeft, function() {
        DOM.write(function() {
     ...
     */
    function recalcChartScale(width, left, cb) {
      ...
      DOM.write(function() {
        ...
        DOM.read(function() {
          ...
          setTimeout(function() {
            ...
            for (var i = 0; i < self.visibleLines.length; i++) {
              DOM.write(function(key) {
                self.nodes.dots[key].style.opacity = 1;
                self.nodes.dots[key].style.transform = 'translate(' + (roundedVerticalLinePosition + ROOT_PADDING) + 'px,-' + (self.chartCellHeight * self.lines[key].values[roundedElementIndex] * self.bounds.chart.yScale + ROOT_PADDING) + 'px)';
              }, self.visibleLines[i]);
            }
  ...
  Chartman.prototype.rescaleXAxis = function(scale) {
    ...
    for (var i = 0; i < self.horizontalLabels.length; i++) {
      ...
      DOM.write(function(i, xOffset, opacity) {
  ...
  /**
   * Submodule that makes it possible to run batch DOM operations in animation frames
   * @type {object}
   */
  var DOM = {
    // actually there should be sort of linked list, but it looks like usually there is no more that 10-20 elements in queue
    // so array is pretty fine
    _readQueue: [],
    _writeQueue: [],

    /**
     * Schedule next run of read-write operations inside requestAnimationFrame
     */
    ...
    /**
     * Run read-write operations from queues
     */
    _run: function() {
      var op;

      // run read operations
      while (op = this._readQueue.shift()) op();

      // run write operations
      while (op = this._writeQueue.shift()) op();

      this._isRunScheduled = false;

      // check if something left and schedule again
      if (this._readQueue.length || this._writeQueue.length) {
        this._scheduleRun();
      }
    },

    /**
     * Add operation to read queue
     * @param {function} cb - Function that makes some read DOM operations
     * @param {...*?} args - An argument for `cb`
     */
    read: function(cb, args) {
      if (arguments.length > 1) {
        this._readQueue.push(cb.apply.bind(cb, null, [].slice.call(arguments, 1)))
      } else {
     ...
     * @param {function} cb - Function that makes some write DOM operations
     * @param {...*?} arg - An argument for `cb`
     */
    write: function(cb, arg) {
      if (arguments.length > 1) {
        this._writeQueue.push(cb.apply.bind(cb, null, [].slice.call(arguments, 1)))
      } else {
