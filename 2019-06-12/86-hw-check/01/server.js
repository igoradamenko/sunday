...
// ensure we have files dir
try {
  fs.statSync(filesDir)
} catch(e) {
  if (e.code === 'ENOENT') {
  ...
  switch (req.method) {
    case 'GET':
      if (pathname === '/' || pathname === '/index.html') {
        ...
        filepath = getSafeFilepath(filename, res);
    ...
    case 'POST':
      ...
      filepath = getSafeFilepath(filename, res);
      if (filepath) return saveFile(filepath, req, res);
      break;
...
function getSafeFilepath(filename, res) {
  ...
  if (!filepath.startsWith(filesDir) || path.dirname(filepath) !== filesDir) { /* 5 */
    return answer(null, res, 400); /* 2 */
  }
